...
- Stage boundaries sit where the human naturally pauses to check — surfacing the judgment call (an outline, a structural plan) as an editable file _before_ the expensive downstream work is the whole trick. Correction is cheapest at the earliest gate.
...
- A pipeline may host sibling _patterns_ (two variants of the same line, e.g. record-then-cut vs animation-first) — the routing move recursing one level down.
...
- **A new record is a copy, not a blank page.** The template _is_ the schema.
