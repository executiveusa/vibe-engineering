...
The consequence, stated once: _stage sequencing is the folder numbering; context scoping is the folder hierarchy; state management is the files on disk; coordination is one folder's output being another folder's input._ The filesystem does the work a framework would do in code.
...
- **Automated mid-pipeline branching** — a human choosing stage 3a vs 3b between stages is natural; the _system_ branching on AI output mid-run pushes ICM toward becoming the framework it replaced.
