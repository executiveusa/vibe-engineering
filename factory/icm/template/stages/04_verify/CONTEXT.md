...
2. Freeze the exact candidate revision and automatically run the pinned `executiveusa/open-code-review` engine after the build. Store `open-code-review.json` with engine revision, base SHA, candidate SHA, command, result, and output hash. Any missing, failing, or stale receipt is a HOLD. Re-run on the final exact revision after every repair.
3. Use browser/runtime evidence for browser products, security review for sensitive surfaces, and measurement before performance optimization.
4. Run the no-slop check on every changed user-facing or architectural surface. A detector finding is evidence to inspect, not an automatic rewrite order.
5. For user-facing work, run the Subtraction Gauntlet against the actual artifact. Classify material elements as KEEP, MERGE, INFER, DEFER, MOVE, or REMOVE. Require the critic to name the protected quality that stops further subtraction.
6. When a reference bar exists, use a fresh reviewer to inspect the actual output next to the real reference and name the largest remaining gap. Do not let the builder perform the only comparison.
