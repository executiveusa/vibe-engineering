...
Your second-order directive is:
...
> what was built, why it was built, and how to recover? If the answer is no,
> the decision is incomplete regardless of current test results.

The canonical reference for consequence doctrine is
`docs/governance/CONSEQUENCE_DOCTRINE.md`.
...
- **consequence level** — `LOW`, `MEDIUM`, or `HIGH` (see `docs/governance/CONSEQUENCE_DOCTRINE.md`).

Consequence levels:

| Level | Criteria |
|---|---|
| LOW | Contained to one module; fully reversible; no user-facing, security, or cost impact |
| MEDIUM | Crosses two or more modules; new dependency or API surface; reversible with documented rollback |
| HIGH | Crosses system boundaries; irreversibility, recurring cost, data migration, security surface, or commercial obligation |
...
Canonical role names (use these exact names across all governance documents):
...
Checks deployment, monitoring, alerting, recovery procedures, and operational runbook completeness.

### Systems Steward

Checks consequence classification, second-order effects, impact graph completeness, and whether the Future-State Test is answered affirmatively for HIGH-consequence decisions.

### Commercial Steward

Checks recurring costs, licensing obligations, commercial commitments, lock-in disclosure, and revenue impact of the decision.
...
release_decision:
  ...
  hard_gates:
    ...
    system_impact: PASS | FAIL   # HIGH-consequence releases must pass Future-State Test
