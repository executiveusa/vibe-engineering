...
// The exact House commit this project is pinned to (null when installed from a packed tarball).
...
export async function installVibe({ target, force = false, skills = true }, sourceRoot) {
  ...
  await writeManagedFile(root, 'CLAUDE.md', '# Vibe Engineering\n\nRead `AGENTS.md`, then `ICMR.yaml`, then `CONTEXT.md`, then `.vibe/VIBE_HOUSE_STANDARD.md`. Use `.vibe/skills/vibe-engineering/SKILL.md` as the canonical house router when present. Use the House version pinned in `.vibe/manifest.json`; upgrading it is a reviewed change, never a mid-task refresh. Verify It Before Everything.\n', force, report);
  await writeManagedFile(root, '.cursor/rules/vibe.mdc', '---\ndescription: Vibe Engineering house law\nalwaysApply: true\n---\n\nRead AGENTS.md, then ICMR.yaml, then CONTEXT.md, then .vibe/VIBE_HOUSE_STANDARD.md. Use .vibe/skills/vibe-engineering/SKILL.md as the canonical router when present. Use the House version pinned in .vibe/manifest.json; upgrading it is a reviewed change, never a mid-task refresh. Verify It Before Everything.\n', force, report);
  await writeManagedFile(root, '.vibe/OPENCODE.md', '# OpenCode adapter\n\nUse the repository filesystem as canonical context. Read `AGENTS.md → ICMR.yaml → CONTEXT.md → .vibe/VIBE_HOUSE_STANDARD.md`, then route material work through `.vibe/skills/vibe-engineering/SKILL.md` when present. Use the House version pinned in `.vibe/manifest.json`; upgrading it is a reviewed change, never a mid-task refresh. Do not create a parallel agent architecture.\n', force, report);
  await writeManagedFile(root, '.vibe/CODEX.md', '# Codex adapter\n\nCodex should enter through root `AGENTS.md`, then read `ICMR.yaml`, `CONTEXT.md`, and `.vibe/VIBE_HOUSE_STANDARD.md`, then use `.vibe/skills/vibe-engineering/SKILL.md` as the canonical house router when present. Use the House version pinned in `.vibe/manifest.json`; upgrading it is a reviewed change, never a mid-task refresh.\n', force, report);
...
async function main() {
  ...
  console.log('Next: tell your agent — "Use Vibe Engineering. Continue from current ICM state on the pinned House version."');
}
